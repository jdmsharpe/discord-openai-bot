...
class ChatGPT(commands.Cog):
    ...
    @slash_command(name='chat', description='Post query to ChatGPT', guild_ids=GUILD_IDS)
    @option('query', description='Prompt', required=True)
    @option('model', description='Choose a GPT model', required=False, choices=[
    ...
    async def chat(self, ctx: ApplicationContext, query: str, model: str='gpt-4-turbo'):
        ...
        try:
            ...
            await ctx.followup.send(embed=Embed(title='ChatGPT Response', description=response_text, color=Colour.blue()))
        except Exception as e:
            await ctx.followup.send(embed=Embed(title='Error', description=str(e), color=Colour.red()))